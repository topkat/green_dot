...
const { apiCall, init, onError, getServerState, setHeaders, beforeApiCall, registerSideEffect, methodNames } = require('./apiCall.cjs')
const { useSuspenseQuery } = require('@tanstack/react-query')
const { ensureAccessToken, getActualScreen, getHandleErrorTyped, initTrackerData, sendDataToServer, trackerEvent, handleError, initErrorHandler, log, initHelpers, logout, setAccessToken } = require('./sdkHelpers')
const { initBackend, isBackendInitialized } = require('./sdkHelpers/initBackend')
...
const $ = new Proxy({
    init: (props) => {
        ...
        init({ ...otherProps, headers: { apiKey, platform: 'icoDashboard' } })
    },
...
$.addQueriesToInvalidate.addTransactionInfo(['icoTransaction*'])
$.addQueriesToInvalidate.updateEmail(['user*'])
$.addQueriesToInvalidate.sendValidationEmail(['user*'])
$.addQueriesToInvalidate.sendUpdateEmail(['user*'])
$.addQueriesToInvalidate.createIcoTransaction(['icoTransaction*', 'icoRewardTransaction*', 'icoDashboardConfig*', 'user*'])
$.addQueriesToInvalidate.cancelTransactionByUser(['icoTransaction*'])
$.addQueriesToInvalidate.addTransactionBonusCode(['icoTransaction*', 'user*'])

/** /!\ Do not export individually (commonJs compatible) */
module.exports = { $, methodNames, getActualScreen, initTrackerData, sendDataToServer, trackerEvent, log, getHandleErrorTyped, handleError, initErrorHandler, ensureAccessToken, initBackend, logout, setAccessToken, isBackendInitialized }