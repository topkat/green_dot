...
import { AuthenticationMethod } from '../../types/core.types'
import { db } from '../../db'
...
export const secureAuth = {
  async compareAndAddAttempt(ctx: Ctx, type: AuthenticationMethod, token: string, userOrId: ModelTypes['user'] | string): Promise<void> {
    ...
    const timeBetweenTwoAttempts = (mainConfig.resetTimeMinutesForSecureConnexion || 15) * 60 * 1000
    ...
    const allowedAttemptsNb = mainConfig?.nbAttemptsForAuth?.[type] || 3
